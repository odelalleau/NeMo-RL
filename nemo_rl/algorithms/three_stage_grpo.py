...
def extract_parallel_thinking_log_data(
    ...
    stage0_repeated_batch: BatchedDataDict[DatumSpec],
    stage0_rewards: torch.Tensor,
    ...
    """Extract comprehensive logging data for three-stage parallel thinking training.
    
    Returns a dictionary with stage0/stage1/stage2 prompts, responses, and rewards for each original prompt.
    """
    ...
    # Extract stage 0 data (first response for each original prompt)
    stage0_prompts = []
    stage0_responses = []
    stage0_rewards_first = []
    
    for i in range(num_original_prompts):
        first_gen_idx = i * num_generations_per_prompt
        prompt, response = extract_prompt_response(stage0_repeated_batch["message_log"][first_gen_idx])
        stage0_prompts.append(prompt)
        stage0_responses.append(response)
        stage0_rewards_first.append(stage0_rewards[first_gen_idx].item())
    ...
    # Create log data with three-stage parallel thinking fields
    log_data = {
        "stage0_prompt": stage0_prompts,
        "stage0_response": stage0_responses,
        "stage0_reward": stage0_rewards_first,
...
class ParallelThinkingGRPOConfig(TypedDict):
    ...
    # Three-stage parallel thinking specific configuration
    stage1_prompt_template: str  # Template for Stage 1 judgment generation
    aggregation_prompt_template: str  # Template for aggregation prompts (Stage 2)
    skip_stage0_env_post_processing: bool # Skip environment post-processing for stage 0
...
def create_stage1_prompts(
    original_batch: BatchedDataDict[DatumSpec],
    stage0_guidance: List[str],
    stage1_prompt_template: str,
    tokenizer,
) -> BatchedDataDict[DatumSpec]:
    """Create Stage 1 prompts for generating judgments using Stage 0 guidance.
    
    Args:
        original_batch: Original batch containing the prompts
        stage0_guidance: List of Stage 0 evaluation guidance for each prompt
        stage1_prompt_template: Template string for Stage 1 judgment generation
        tokenizer: Tokenizer to use for tokenizing the prompts
    
    Returns:
        New batch with Stage 1 prompts
    """
    stage1_batch = deepcopy(original_batch)
    
    # Create new message logs for Stage 1
    new_message_logs = []
    new_extra_env_info = []
    new_loss_multiplier = []
    new_task_name = []
    ...
    for i, message_log in enumerate(original_batch["message_log"]):
        # Skip if no guidance for this prompt
        if i >= len(stage0_guidance) or not stage0_guidance[i]:
            continue
        ...
        original_prompt = original_batch["extra_env_info"][i].get("conversation_context")
        if original_prompt is None:
            raise ValueError(f"No question found in metadata for sample {i}")
        
        # Create the Stage 1 prompt using the template
        stage1_prompt = stage1_prompt_template.format(
            original_prompt=original_prompt,
            evaluation_guidance=stage0_guidance[i]
        )
        ...
        # Create new message log with the Stage 1 prompt
        new_message_log = [
            {
                "role": "user",
    ...
    # Ensure we have at least some valid Stage 1 prompts
    if len(new_message_logs) == 0:
        raise ValueError("No valid Stage 1 prompts could be created from the Stage 0 guidance")
    
    print(f"  ✓ Created {len(new_message_logs)} Stage 1 prompts from {len(original_batch['message_log'])} original prompts")
    
    # Update Stage 1 batch with new data
    stage1_batch["message_log"] = new_message_logs
    stage1_batch["extra_env_info"] = new_extra_env_info
    stage1_batch["loss_multiplier"] = torch.tensor(new_loss_multiplier)
    stage1_batch["task_name"] = new_task_name
...
def combine_and_shuffle_training_data(
    stage0_train_data: BatchedDataDict[ClippedPGLossDataDict],
    ...
    """Combine training data from all three stages and shuffle.
    
    Note: Different stages may have different sequence lengths, so we pad to match the longest.
    The combined data is shuffled to mix stage 0, stage 1 and stage 2 samples.
    """
    
    # Get sequence lengths
    stage0_seq_len = stage0_train_data["input_ids"].shape[1]
    stage1_seq_len = stage1_train_data["input_ids"].shape[1]
    stage2_seq_len = stage2_train_data["input_ids"].shape[1]
    
    # Find the maximum sequence length
    max_seq_len = max(stage0_seq_len, stage1_seq_len, stage2_seq_len)
    
    # Pad all stages to the maximum sequence length
    if stage0_seq_len < max_seq_len:
        pad_size = max_seq_len - stage0_seq_len
        stage0_train_data["input_ids"] = torch.nn.functional.pad(
            stage0_train_data["input_ids"], (0, pad_size), value=0
        )
        stage0_train_data["advantages"] = torch.nn.functional.pad(
            stage0_train_data["advantages"], (0, pad_size), value=0
        )
        stage0_train_data["generation_logprobs"] = torch.nn.functional.pad(
            stage0_train_data["generation_logprobs"], (0, pad_size), value=0
        )
        stage0_train_data["token_mask"] = torch.nn.functional.pad(
            stage0_train_data["token_mask"], (0, pad_size), value=0
        )
    
    if stage1_seq_len < max_seq_len:
        pad_size = max_seq_len - stage1_seq_len
    ...
    if stage2_seq_len < max_seq_len:
        pad_size = max_seq_len - stage2_seq_len
    ...
    # Now concatenate the tensors (all have same sequence length)
    combined_data = BatchedDataDict[ClippedPGLossDataDict]({
        "input_ids": torch.cat([stage0_train_data["input_ids"], stage1_train_data["input_ids"], stage2_train_data["input_ids"]], dim=0),
        "input_lengths": torch.cat([stage0_train_data["input_lengths"], stage1_train_data["input_lengths"], stage2_train_data["input_lengths"]], dim=0),
        "advantages": torch.cat([stage0_train_data["advantages"], stage1_train_data["advantages"], stage2_train_data["advantages"]], dim=0),
        "generation_logprobs": torch.cat([stage0_train_data["generation_logprobs"], stage1_train_data["generation_logprobs"], stage2_train_data["generation_logprobs"]], dim=0),
        "token_mask": torch.cat([stage0_train_data["token_mask"], stage1_train_data["token_mask"], stage2_train_data["token_mask"]], dim=0),
        "sample_mask": torch.cat([stage0_train_data["sample_mask"], stage1_train_data["sample_mask"], stage2_train_data["sample_mask"]], dim=0),
    })
    
    # Shuffle the combined data to mix stage 0, stage 1 and stage 2 samples
...
def parallel_thinking_grpo_train(
    ...
    while step < max_num_steps and not time_limit_timer.is_finished():
        ...
        with timer.time("total_step_time"):
            # ============== Stage 0: Generate Evaluation Guidance ==============
            print("\n▶ Stage 0: Generate Evaluation Guidance...")
            with timer.time("stage0_preparation"):
                # Stage 0 prompts come directly from the data file (already formatted)
                stage0_batch = batch
                # Repeat batch items for multiple generations
                stage0_repeated_batch: BatchedDataDict[DatumSpec] = stage0_batch.repeat_interleave(
                    master_config["pt_grpo"]["num_generations_per_prompt"]
                )
                # Convert LLMMessageLogType to FlatMessagesType for generation
                stage0_flat_pre_gen, stage0_input_lengths = batched_message_log_to_flat_message(
                    stage0_repeated_batch["message_log"],
                    pad_value_dict={"token_ids": tokenizer.pad_token_id},
                )
                stage0_prompt_input_ids = stage0_flat_pre_gen["token_ids"]

            # Generate Stage 0 evaluation guidance
            print(f"  • Generating {stage0_repeated_batch.size} Stage 0 evaluation guidance responses...")
            with timer.time("stage0_generation_prep"):
                if NEED_REFIT and POLICY_GENERATION_STALE:
            ...
            with timer.time("stage0_generation"):
                stage0_repeated_batch, stage0_rollout_metrics = run_multi_turn_rollout(
                    policy_generation=policy_generation,
                    input_batch=stage0_repeated_batch,
                    tokenizer=tokenizer,
                ...
                # Keep generation active for next stages

            # Apply environment post-processing for stage 0
            print("  • Applying stage 0 environment post-processing...")
            with timer.time("stage0_env_post_processing"):
                if master_config["pt_grpo"].get("skip_stage0_env_post_processing", False):
                    stage0_env_metrics = {}
                    print("    (Skipped - skip_stage0_env_post_processing is enabled)")
                else:
                    stage0_repeated_batch, stage0_env_metrics = apply_environment_post_processing(
                        stage0_repeated_batch, task_to_env, prefix="stage0_"
                    )
                stage0_rollout_metrics.update(stage0_env_metrics)

            # Extract Stage 0 guidance responses grouped by original prompt
            num_prompts = len(batch["message_log"])  # Number of original prompts
            num_generations = master_config["pt_grpo"]["num_generations_per_prompt"]
            
            # Stage 0: num_prompts original prompts -> num_prompts * num_generations Stage 0 guidances
            stage0_guidance_by_prompt = []
            for i in range(num_prompts):
                prompt_guidance = []
                for j in range(num_generations):
                    idx = i * num_generations + j
                    # Extract assistant response from the message log
                    for message in stage0_repeated_batch["message_log"][idx]:
                        if message["role"] == "assistant":
                            prompt_guidance.append(message["content"])
                            break
                stage0_guidance_by_prompt.append(prompt_guidance)

            # ============== Stage 1: Generate Judgments Using Stage 0 Guidance ==============
            print("\n▶ Stage 1: Generate Judgments...")
            with timer.time("stage1_preparation"):
                # Create Stage 1 prompts for each Stage 0 guidance
                # We'll create multiple Stage 1 batches, one for each Stage 0 guidance
                # Stage 1: num_prompts * num_generations Stage 0 guidances -> 
                #          (num_prompts * num_generations) * num_generations Stage 1 judgments
                all_stage1_batches = []
                stage0_to_stage1_mapping = []  # Track which Stage 0 guidance maps to which Stage 1 responses
                
                for prompt_idx in range(num_prompts):
                    for guidance_idx, guidance in enumerate(stage0_guidance_by_prompt[prompt_idx]):
                        # Create a batch with this specific guidance
                        single_guidance_list = [guidance]
                        stage1_batch = create_stage1_prompts(
                            batch.select_indices([prompt_idx]),
                            single_guidance_list,
                            master_config["pt_grpo"]["stage1_prompt_template"],
                            tokenizer,
                        )
                        # Repeat for multiple generations per guidance
                        repeated_batch = stage1_batch.repeat_interleave(
                            master_config["pt_grpo"]["num_generations_per_prompt"]
                        )
                        all_stage1_batches.append(repeated_batch)
                        # Track mapping: (prompt_idx, stage0_guidance_idx) -> stage1_batch_idx
                        stage0_to_stage1_mapping.append((prompt_idx, guidance_idx))
                
                # Concatenate all Stage 1 batches
                stage1_repeated_batch = BatchedDataDict.concatenate(all_stage1_batches)
                
                # Convert LLMMessageLogType to FlatMessagesType for generation
                stage1_flat_pre_gen, stage1_input_lengths = batched_message_log_to_flat_message(
                    stage1_repeated_batch["message_log"],
                    pad_value_dict={"token_ids": tokenizer.pad_token_id},
                )
                stage1_prompt_input_ids = stage1_flat_pre_gen["token_ids"]

            # Generate Stage 1 judgments
            print(f"  • Generating {stage1_repeated_batch.size} Stage 1 judgment responses...")
            ...
            # ============== Calculate Stage 0 Rewards ==============
            # Stage 0 rewards are the average of Stage 1 rewards for each guidance
            print("\n▶ Calculating Stage 0 rewards based on Stage 1 performance...")
            with timer.time("stage0_reward_calculation"):
                stage0_rewards = torch.zeros(stage0_repeated_batch.size)
                
                # Group Stage 1 rewards by Stage 0 guidance
                current_idx = 0
                for prompt_idx in range(num_prompts):
                    for guidance_idx in range(num_generations):
                        # Get all Stage 1 rewards for this Stage 0 guidance
                        stage1_start_idx = current_idx
                        stage1_end_idx = current_idx + num_generations
                        guidance_stage1_rewards = stage1_rewards[stage1_start_idx:stage1_end_idx]
                        
                        # Calculate average reward for this Stage 0 guidance
                        avg_reward = guidance_stage1_rewards.mean()
                        
                        # Assign this average to the corresponding Stage 0 response
                        stage0_idx = prompt_idx * num_generations + guidance_idx
                        stage0_rewards[stage0_idx] = avg_reward
                        
                        current_idx = stage1_end_idx
                
                # Update Stage 0 batch with calculated rewards
                stage0_repeated_batch["total_reward"] = stage0_rewards
                
                print(f"  • Stage 0 avg reward: {stage0_rewards.mean():.4f}")
            ...
            with timer.time("stage2_preparation"):
                ...
                # Group Stage 1 responses by Stage 0 guidance (not by original prompt)
                # Stage 2: For each of the (num_prompts * num_generations) Stage 0 guidances,
                #          aggregate its num_generations Stage 1 judgments
                # Result: (num_prompts * num_generations) Stage 2 aggregations
                stage2_aggregation_batches = []
                stage1_responses_by_guidance = []
                
                # Process each Stage 0 guidance
                for guidance_idx, (prompt_idx, stage0_guidance_idx) in enumerate(stage0_to_stage1_mapping):
                    # Get the Stage 1 responses for this specific Stage 0 guidance
                    stage1_start_idx = guidance_idx * num_generations
                    stage1_end_idx = stage1_start_idx + num_generations
                    
                    guidance_stage1_responses = []
                    for idx in range(stage1_start_idx, stage1_end_idx):
                        # Extract assistant response from the message log
                        ...
                        if last_assistant_response is not None:
                            if reasoning_split_word and reasoning_split_word in last_assistant_response:
                                ...
                                guidance_stage1_responses.append(last_assistant_response.split(reasoning_split_word)[-1].lstrip())
                            else:
                                guidance_stage1_responses.append(last_assistant_response)
                    
                    # Randomly select a subset of responses for aggregation
                    # Only select powers of 2: 1, 2, 4, 8, etc.
                    if guidance_stage1_responses:
                        num_to_select = random.choice([2**i for i in range(len(guidance_stage1_responses).bit_length()) if 2**i <= len(guidance_stage1_responses)])
                        selected_responses = random.sample(guidance_stage1_responses, num_to_select)
                        stage1_responses_by_guidance.append(selected_responses)
                        
                        # Create aggregation prompt for this Stage 0 guidance
                        single_prompt_batch = batch.select_indices([prompt_idx])
                        aggregation_batch = create_aggregation_prompts(
                            single_prompt_batch,
                            [selected_responses],
                            master_config["pt_grpo"]["aggregation_prompt_template"],
                            tokenizer,
                        )
                        stage2_aggregation_batches.append(aggregation_batch)
                
                # Concatenate all Stage 2 aggregation batches
                aggregation_batch_template = BatchedDataDict.concatenate(stage2_aggregation_batches)
            ...
            # Calculate stage2_better_than_stage1_avg_rate metric
            # Stage 2 aggregates Stage 1 judgments for each Stage 0 guidance
            num_stage0_guidances = len(stage0_to_stage1_mapping)
            stage2_num_prompts = stage2_rewards.numel() // num_generations if stage2_rewards.numel() > 0 else 0
            
            if stage2_num_prompts != num_stage0_guidances:
                raise ValueError(
                    f"Stage 2 prompt count ({stage2_num_prompts}) does not match Stage 0 guidance count ({num_stage0_guidances}). "
                    "This suggests an issue with the aggregation process."
                )

            # Group Stage 1 rewards by Stage 0 guidance to compare with Stage 2
            stage1_rewards_by_guidance = []
            current_idx = 0
            for guidance_idx in range(num_stage0_guidances):
                guidance_start_idx = current_idx
                guidance_end_idx = current_idx + num_generations
                guidance_rewards = stage1_rewards[guidance_start_idx:guidance_end_idx]
                stage1_rewards_by_guidance.append(guidance_rewards)
                current_idx = guidance_end_idx

            # Group Stage 2 rewards by guidance
            stage2_rewards_by_guidance = stage2_rewards.view(num_stage0_guidances, num_generations)

            # Compare each Stage 2 response to the average of its corresponding Stage 1 responses
            stage2_better_than_stage1_count = 0
            total_comparisons = 0
            
            for guidance_idx in range(num_stage0_guidances):
                stage1_avg_for_guidance = stage1_rewards_by_guidance[guidance_idx].mean()
                stage2_rewards_for_guidance = stage2_rewards_by_guidance[guidance_idx]
                
                # Count how many Stage 2 responses beat the Stage 1 average for this guidance
                stage2_better_than_stage1_count += (stage2_rewards_for_guidance > stage1_avg_for_guidance).sum().item()
                total_comparisons += num_generations
            
            stage2_better_than_stage1_avg_rate = stage2_better_than_stage1_count / total_comparisons if total_comparisons > 0 else 0.0

            # ============== Calculate Rewards & Advantages ==============
            print("\n▶ Processing rewards and advantages...")
            with timer.time("reward_calculation"):
                # Stage 0 advantages
                print("  • Computing stage 0 advantages...")
                ...
                stage0_baseline, stage0_std, stage0_metrics = calculate_baseline_and_std_per_prompt(
                    stage0_prompt_input_ids,
                    stage0_rewards,
                    torch.ones_like(stage0_rewards),
                    leave_one_out_baseline=master_config["pt_grpo"]["use_leave_one_out_baseline"],
                    expected_responses_per_prompt=expected_responses,
                )
                stage0_advantages = (stage0_rewards - stage0_baseline).unsqueeze(-1)

                # Stage 1 advantages
                print("  • Computing stage 1 advantages...")
                
                # For Stage 1, we have multiple responses per Stage 0 guidance
                # So we need to handle the baseline calculation differently
                ...
                if master_config["pt_grpo"]["normalize_rewards"]:
                    ...
                    zero_std_mask = stage0_std > 0
                    stage0_advantages[zero_std_mask] = (
                        stage0_advantages[zero_std_mask] / stage0_std.unsqueeze(-1)[zero_std_mask]
                    )
                ...
                all_rewards = torch.cat([stage0_rewards, stage1_rewards, stage2_rewards])
                all_advantages = torch.cat([stage0_advantages.flatten(), stage1_advantages.flatten(), stage2_advantages.flatten()])
                
                # Calculate metrics
                rollout_metrics = {}
                rollout_metrics.update({"stage0_" + k: v for k, v in stage0_rollout_metrics.items()})
                ...
                rollout_metrics.update({"stage0_" + k: v for k, v in stage0_metrics.items()})
                ...
                rollout_metrics.update({
                    "stage0_reward_min": stage0_rewards.min(),
                    "stage0_reward_mean": stage0_rewards.mean(),
                    "stage0_reward_max": stage0_rewards.max(),
                    "stage0_baseline_mean": stage0_baseline.mean(),
                    "stage0_std_mean": stage0_std.mean(),
            ...
            with timer.time("data_processing"):
                # Prepare stage 0 training data
                for i, message_log in enumerate(stage0_repeated_batch["message_log"]):
                    for j, message in enumerate(message_log):
                        ...
                        message["advantages"] = stage0_advantages[i].expand(message["token_ids"].shape)

                # Convert stage 0 to training data
                stage0_flat_messages, stage0_input_lengths = batched_message_log_to_flat_message(
                    stage0_repeated_batch["message_log"],
                    pad_value_dict={"token_ids": tokenizer.pad_token_id},
                ...
                stage0_train_data = BatchedDataDict[ClippedPGLossDataDict]({
                    "input_ids": stage0_flat_messages["token_ids"],
                    "input_lengths": stage0_input_lengths,
                    "advantages": stage0_flat_messages["advantages"],
                    "generation_logprobs": stage0_flat_messages["generation_logprobs"],
                    "token_mask": stage0_flat_messages["token_loss_mask"],
                    "sample_mask": stage0_repeated_batch["loss_multiplier"],
                })
                ...
                # Combine and shuffle training data from all three stages
                train_data = combine_and_shuffle_training_data(stage0_train_data, stage1_train_data, stage2_train_data)
                train_data.to("cpu")
        ...
        print(f"  • Stage 0 Mean Gen Length: {rollout_metrics.get('stage0_mean_gen_tokens_per_sample', 0):.1f}")
        ...
        # Use stage0_flat_messages content for logging
        log_data = {"content": stage0_flat_messages["content"]}
        log_data["rewards"] = stage0_rewards.tolist()
        log_data["dataset_names"] = stage0_repeated_batch.get("dataset_names", ["default"] * len(stage0_rewards))
        
        # Add three-stage parallel thinking specific data
        pt_data = extract_parallel_thinking_log_data(
            batch=batch,
            stage0_repeated_batch=stage0_repeated_batch,
            stage0_rewards=stage0_rewards,
        ...
        # Log table with three-stage parallel thinking data
