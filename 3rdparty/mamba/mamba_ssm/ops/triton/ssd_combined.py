...
def ensure_stride1_at_dim1_bds(t: Tensor) -> Tensor:
    """Ensure a 3D tensor shaped as (batch, dim1, dim2) has stride(1) == 1.

    For inputs we pass to causal_conv1d_bwd_function we use layout (b, d, s).
    That CUDA op requires the middle dimension to be contiguous (stride(1) == 1).
    Calling .contiguous() on (b, d, s) makes stride of the last dim 1, not dim 1.
    The trick is to materialize as (b, s, d).contiguous() and then view back as (b, d, s),
    which yields stride(1) == 1 while preserving the expected shape.
    """
    assert t.dim() == 3, "Expected 3D tensor"
    if t.stride(1) == 1:
        return t
    t_bsd = rearrange(t, "b d s -> b s d").contiguous()
    t_bds = rearrange(t_bsd, "b s d -> b d s")
    return t_bds
...
class MambaSplitConv1dScanCombinedFn(torch.autograd.Function):
    ...
    def forward(ctx, zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, chunk_size, initial_states=None, seq_idx=None, dt_limit=(0.0, float("inf")), return_final_states=False, activation="silu",
        ...
        # Pad channels when using varlen (seq_idx) so the CUDA conv path supports it.
        conv_ch = conv1d_weight.shape[0]  # == dim + 2 * ngroups * dstate
        pad_ch = (128 - (conv_ch % 128)) % 128
        use_pad = (seq_idx is not None) and (pad_ch != 0)
        if use_pad:
            # Pad last dim of xBC (channels) and first dim of weight/bias (depthwise groups)
            xBC_pad = F.pad(xBC, (0, pad_ch))
            w_pad = F.pad(conv1d_weight, (0, 0, 0, pad_ch))
            b_pad = F.pad(conv1d_bias, (0, pad_ch)) if conv1d_bias is not None else None
        else:
            xBC_pad, w_pad, b_pad = xBC, conv1d_weight, conv1d_bias
        xBC_conv = rearrange(
            causal_conv1d_fwd_function(rearrange_and_update_stride(xBC_pad, "b s d -> b d s"),
                                                w_pad, b_pad, seq_idx, None, None, activation in ["silu", "swish"]),
            "b d s -> b s d"
        )
        if use_pad:
            # Remove padded channels before splitting into x, B, C
            xBC_conv = xBC_conv[..., :conv_ch]
    ...
    def backward(ctx, dout, *args):
        ...
        xBC_conv = rearrange(
            causal_conv1d_fwd_function(rearrange_and_update_stride(xBC_pad, "b s d -> b d s"),
                                       w_pad, b_pad, seq_idx, None, None, ctx.activation in ["silu", "swish"]),
            "b d s -> b s d"
        )
        if use_pad:
            xBC_conv = xBC_conv[..., :conv_ch]
        ...
        # Backward through padded conv
        if use_pad:
            # Prepare padded tensors in channel-first (b, d, s)
            xBC_bwd = ensure_stride1_at_dim1_bds(rearrange_and_update_stride(xBC_pad, "b s d -> b d s"))
            dxBC_bwd_tmp = rearrange(dxBC, "b s d -> b d s")
            # dxBC_given is already rearranged to (b, d, s) above
            dxBC_given_bwd_tmp = dxBC_given
            if pad_ch > 0:
                # Pad channel dimension (middle dim)
                dxBC_bwd_tmp = F.pad(dxBC_bwd_tmp, (0, 0, 0, pad_ch))
                dxBC_given_bwd_tmp = F.pad(dxBC_given_bwd_tmp, (0, 0, 0, pad_ch))
            # Ensure stride(1) == 1 after padding
            dxBC_bwd = ensure_stride1_at_dim1_bds(dxBC_bwd_tmp)
            dxBC_given_bwd = ensure_stride1_at_dim1_bds(dxBC_given_bwd_tmp)
            dxBC_given_update, dweight, dbias, *_ = causal_conv1d_bwd_function(
                xBC_bwd, w_pad, b_pad, dxBC_bwd, seq_idx, None, None, dxBC_given_bwd, False, ctx.activation in ["silu", "swish"]
            )
            # Crop updates and grads back to original channel count
            dxBC_given_update = dxBC_given_update[:, :conv_ch, :]
            dweight = dweight[:conv_ch, ...]
