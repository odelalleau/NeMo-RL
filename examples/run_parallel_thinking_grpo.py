...
from nemo_rl.algorithms.parallel_thinking_grpo import MasterConfig, parallel_thinking_grpo_train, setup
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Run Parallel Thinking GRPO training with configuration")
    parser.add_argument(
...
def main():
    ...
    if not args.config:
        args.config = os.path.join(os.path.dirname(__file__), "configs", "parallel_thinking_grpo_1B.yaml")
    ...
    if (
        config["pt_grpo"]["num_prompts_per_step"]
        * config["pt_grpo"]["num_generations_per_prompt"]
        != config["policy"]["train_global_batch_size"]
        and config["policy"]["num_global_batch_repeats"] > 1
    ):
        raise ValueError(
            f"num prompts per step ({config['pt_grpo']['num_prompts_per_step']}) * "
            f"num generations per prompt ({config['pt_grpo']['num_generations_per_prompt']}) "
    ...
    # Validate parallel thinking specific configuration
    if "aggregation_prompt_template" not in config["pt_grpo"]:
        raise ValueError("pt_grpo.aggregation_prompt_template must be specified for parallel thinking GRPO")
