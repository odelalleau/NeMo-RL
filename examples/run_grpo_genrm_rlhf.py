...
from nemo_rl.data.hf_datasets.genrm_rlhf import GenRMRLHFDataset
...
def setup_data(
    ...
    task_to_env: dict[str, EnvironmentInterface] = defaultdict(
        lambda: genrm_rlhf_env
    )
...
def main() -> None:
    ...
    if "async_grpo" in config["grpo"] and config["grpo"]["async_grpo"]["enabled"]:
        ...
        print("🚀 Running synchronous GRPO training for GenRM RLHF")
