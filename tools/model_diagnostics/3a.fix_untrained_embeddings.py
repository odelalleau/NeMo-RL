...
def reinit_near_zero_rows(embeddings, near_zero_threshold, reinit_std, method="normal"):
    """Reinitialize embedding rows that are entirely near-zero.

    A row is considered near-zero if all absolute values are < ``near_zero_threshold``.
    Two reinitialization strategies are supported:
    - ``method='normal'``: initialize each near-zero row from N(0, ``reinit_std``^2).
    - ``method='mean'``: set each near-zero row to the mean vector computed over
      all non–near-zero rows. If no non–near-zero rows exist, fall back to
      the ``'normal'`` strategy.

    Returns
    -------
    List[int]
        The indices of rows that were reinitialized.
    """
    # Resolve to tensor of shape [vocab_size, hidden_size]
    weights = embeddings.weight if hasattr(embeddings, "weight") else embeddings

    with torch.no_grad():
        near_zero_mask = torch.abs(weights.data) < near_zero_threshold
        near_zero_rows = near_zero_mask.all(dim=1)
        indices_tensor = torch.where(near_zero_rows)[0]
        indices = indices_tensor.tolist()
        if not indices:
            return indices

        if method == "mean":
            non_near_zero_rows = (~near_zero_rows)
            if non_near_zero_rows.any():
                mean_vector = weights.data[non_near_zero_rows].mean(dim=0)
                for idx in indices:
                    weights.data[idx].copy_(mean_vector)
            else:
                # Fallback to normal init if all rows are near-zero
                for idx in indices:
                    torch.nn.init.normal_(weights.data[idx], mean=0.0, std=reinit_std)
        else:
            for idx in indices:
                torch.nn.init.normal_(weights.data[idx], mean=0.0, std=reinit_std)
    return indices
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional directory to save modified model and tokenizer",
    )
    parser.add_argument(
        "--reinit-std",
        type=float,
        default=0.014,
        help="Std dev when reinitializing near-zero embeddings (default: 0.014)",
    )
    parser.add_argument(
        "--reinit-method",
        type=str,
        choices=["normal", "mean"],
        default="normal",
        help="Reinitialization method for near-zero rows: 'normal' (default) or 'mean'",
    )
    ...
    # Reinitialize near-zero embeddings before analysis
    input_embeddings = model.get_input_embeddings()
    if input_embeddings is not None:
        input_reinit_indices = reinit_near_zero_rows(
            input_embeddings, args.near_zero_threshold, args.reinit_std, args.reinit_method
        )
        if args.reinit_method == "normal":
            print(
                f"Reinitialized {len(input_reinit_indices)} near-zero rows in Input Embeddings (method=normal, std={args.reinit_std})"
            )
        else:
            print(
                f"Reinitialized {len(input_reinit_indices)} near-zero rows in Input Embeddings (method=mean)"
            )
        if input_reinit_indices:
            print(f"  Indices: {format_index_ranges(input_reinit_indices)}")

    output_embeddings_for_reinit = find_output_embeddings(model)
    if output_embeddings_for_reinit is not None:
        output_reinit_indices = reinit_near_zero_rows(
            output_embeddings_for_reinit, args.near_zero_threshold, args.reinit_std, args.reinit_method
        )
        if args.reinit_method == "normal":
            print(
                f"Reinitialized {len(output_reinit_indices)} near-zero rows in Output Embeddings (method=normal, std={args.reinit_std})"
            )
        else:
            print(
                f"Reinitialized {len(output_reinit_indices)} near-zero rows in Output Embeddings (method=mean)"
            )
        if output_reinit_indices:
            print(f"  Indices: {format_index_ranges(output_reinit_indices)}")
    ...
    # Save modified model/tokenizer (optional)
    if args.save_dir:
        print(f"\nSaving modified model and tokenizer to: {args.save_dir}")
        model.save_pretrained(args.save_dir, safe_serialization=True)
        tokenizer.save_pretrained(args.save_dir)
    else:
        print("\nNo --save-dir provided; skipping save.")
